...
function loadVideo(event) {
	...
	swfobject.embedSWF("http://www.youtube.com/v/" + videoID +
			...
			*/
var customtemp = 1000;
function removeVideo(tempcus){
		alert("removing");
		/*
		vidid = tempcus.slice(0,6);
		
		for ( k=0;k<customplay.length;k++){
  		  if( customplay[k].id == vidid ){
...
function addVideo(event){
	...
	for ( k=0;k<customplay.length;k++){
  		if( customplay[k].id == event.target.id ){
			alert("Was/In custom playlist: Kitni baar Sunega ek hi gaana ??");
			return;
		}
	}		
	...
	//for(i=0;i<customplay.length;i++)
		store1 = store1+ '<div id="custom' + event.target.id   + '"> <li onclick=loadVideo("http://www.youtube.com/v/'+event.target.id +  '?version=3&f=related&app=youtube_gdata",true)>'+ event.target.title  ;
		store1 = store1 + "<img src='http://img.youtube.com/vi/" +  event.target.id +"/1.jpg'></img></li>";
		tempcus = "custom" + event.target.id;
		store1 = store1 + '<div title=' + tempcus + ' onclick=removeVideo("' + tempcus  +'") > Remove </div> </div>';
